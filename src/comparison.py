...
class ComparisonResult:
    def __init__(self,K_word: list[list[str,int]], Q_word: list[str,int]):
        self.K_word = K_word;
        self.Q_word = Q_word;
    def show(self):
        K_word = [[] for i in range(len(self.K_word))]
        #for i in self.K_word:
        #    print(i)
        for i in range(len(self.K_word)):
            K_word[i] = sorted(self.K_word[i], key = lambda x:x[1], reverse = True)
        Q_word = sorted(self.Q_word, key = lambda x:x[1], reverse = True)
        K_max = 0
        for i in K_word:
            K_max = max(K_max,len(i))
        l = (max(K_max,len(Q_word)))
        for i in range(0,l,20):
            ...
            for l in range(len(K_word)):
                print("K"+str(l),end = ' ')
                for j in range(i,i+20):
                    if(j < len(K_word)):
                        print(K_word[l][j], end = ' ')
                print()
...
class Testcomparison(unittest.TestCase):
    def test_compmparison(self):
        ...
        with NamedTemporaryFile(suffix="db") as db, NamedTemporaryFile() as f1,NamedTemporaryFile() as f2,NamedTemporaryFile() as f3:
